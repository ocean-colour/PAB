...
# parse profile observation time for seasonal figure
valid["time_dt"] = pd.to_datetime(valid["time"], utc=True, errors="coerce")
valid["month"]   = valid["time_dt"].dt.month

# basin assignment
def basin(lat, lon):
    if -35 < lat and (lon < -20 or lon > 100):
        return "Atlantic" if lon < -20 else "Pacific"
    if lat <= -35:
        return "Southern"
    if -20 <= lon < 25:
        return "Atlantic"
...
Scripts saved to `pab/matchup/` — see inline code above for the full
data preparation; plotting follows the same data structures.
...
Or run the individual figure scripts once added to `pab/matchup/`.
