...
from both stores. Generated by the MATLAB script
[`docs/scripts/fig_architecture.m`](../scripts/fig_architecture.m). (MATLAB was
not available in the authoring environment, so this PNG has not yet been
rendered — run the script in MATLAB to produce it.)*
